...
class VerLista extends PainelControlador
{
    public function gerar(int $id_lista): void
    {
        $modelo = new ListaModelo();
        $lista_array = (array) $modelo->buscaPorId($id_lista);
    
        $objDados = $lista_array["\0*\0dados"] ?? null;
    
        if (!$objDados || !isset($objDados->lista_completa)) {
            echo "Lista não encontrada ou dados inválidos.";
            return;
        }
        $dados = json_decode($objDados->lista_completa, true);
