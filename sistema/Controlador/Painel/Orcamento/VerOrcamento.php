...
class VerOrcamento extends PainelControlador
{
    public function gerar(int $id_orcamento): void
    {
        $modelo = new OrcamentoModelo();
        $orcamento_array = (array) $modelo->buscaPorId($id_orcamento);
    
        $objDados = $orcamento_array["\0*\0dados"] ?? null;
    
        if (!$objDados || !isset($objDados->orcamento_completo)) {
            echo "Orçamento não encontrado ou dados inválidos.";
            return;
        }
    
        $dados = json_decode($objDados->orcamento_completo, true);
